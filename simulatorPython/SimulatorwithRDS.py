...
def analyzeGraph(fp, numHops):
  ...
  print "RDS"
  RDS = runRDS(startNode, numHops, Graph)
  print RDS
...
def runRDS(startNode, numHops, Graph):
  ...
  while ( len(walkedNodes) < numHops ):
    if (len(currNeighbors) < 4): #if it 3 neighbors or fewer, take all the neighbors
      candidateNodes.append(currNeighbors)
    else: #choosing 3 neighbors randomly
      for j in range (1,3):
        i = random.randint(0, (len(currNeighbors) - j) )
        candidateNodes.append( currNeighbors[i] )
        currNeighbors.append ( currNeighbors.pop(i) )
    #activate 1 out of 3 nodes
    for k in range (0, (len (candidateNodes) - 1)):
      p = random.random()
      if p < 0.66:
        walkedNodes.append(candidateNodes[k])
    #move to the next RDS activated node    
    currNode = walkedNodes[random.randint(0, len(walkedNodes) - 1)]
    currNeighbors = Graph[currNode]
  print "RDS done. RDS nodes: %s"%walkedNodes
  print "Number of nodes RDS activated: %s Number of unique nodes: %s"\
